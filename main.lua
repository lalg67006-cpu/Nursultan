...
-- Вкладки
local TabContents = {}
for _, tabName in ipairs({"Combat","Movement","Visuals","Player","Misc","Presets","Auto Buy","Accounts"}) do
...
local function addSidebarItem(name, iconId, isDefaultActive, order)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        if TabContents[activeTabName] then TabContents[activeTabName].Visible = false end
        activeTabName = name
        if TabContents[activeTabName] then TabContents[activeTabName].Visible = true end
...
-- Создание колонок внутри вкладки
local function createTabColumns(tabName)
    local tab = TabContents[tabName]
